...
test.describe('Filters functionality', () => {
  test.beforeEach(async ({ page, baseURL }) => {
    ...
    await Promise.all([
      ...
      page.waitForSelector('#open-filter-drawer:not([disabled])', { state: 'visible' })
    ]);
  });
  ...
  test('filter drawer closes when close button is clicked', async ({ page }) => {
    ...
    const closeButton = page.getByRole('button', { name: /Show \d+ of \d+ events/ });
  ...
  test('reset button clears filters', async ({ page }) => {
    ...
    await page.getByText('Not accepting talks', { exact: true }).first().click();
    await page.waitForTimeout(100); // Wait for state update
